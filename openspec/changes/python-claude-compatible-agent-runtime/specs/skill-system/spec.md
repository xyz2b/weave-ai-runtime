...
### Requirement: Claude 兼容的 skill 定义
runtime SHALL 支持使用 Claude Code 兼容 `SKILL.md` 形式编写的 skill definitions，包括用于 description、model selection、effort、tool restrictions、hooks、execution mode 与 invocation behavior 的 frontmatter 字段。

#### Scenario: 从 SKILL.md 加载 skill
- **WHEN** runtime 在支持的 skill 目录中发现一个有效的 `SKILL.md` 文件
- **THEN** runtime SHALL 使用 Claude Code 兼容的 frontmatter 语义加载该 skill
...
runtime SHALL 支持 Claude Code 兼容的 skill execution 行为，包括直接 prompt injection，以及通过专用 agent context 执行的 forked execution。
