...
from examples._shared.bootstrap import PROJECT_ROOT
...
def default_layout(*, state_root: Path | None = None) -> CodeAssistantLayout:
    demo_root = PROJECT_ROOT / "examples" / "apps" / "code_assistant"
    env_state_root = os.environ.get(CODE_ASSISTANT_STATE_ROOT_ENV, "").strip()
    return CodeAssistantLayout(
        demo_root=demo_root,
        state_root=state_root
        or (
            Path(env_state_root).expanduser()
            if env_state_root
            else PROJECT_ROOT / ".local" / "examples" / "code_assistant"
        ),
    )
