...
The runtime SHALL discover additional `.runtime/skills` roots by walking upward from observed workspace paths that remain under the session cwd, and SHALL merge those roots into the current session skill view.

#### Scenario: File observation discovers a nested skill root during a turn
- **WHEN** a tool reads, edits, or writes a file inside a subtree that contains a closer `.runtime/skills` directory
