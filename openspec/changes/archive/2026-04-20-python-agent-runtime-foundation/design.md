...
当前仓库已经在 `cc-src` 和 `docs/cc` 中沉淀了大量参考实现架构的源码解读，但还没有把这些内容正式收敛成一个可复用 runtime framework 的设计方案。目标方向现在已经明确：

- 实现语言选择 Python
- 面向用户的 `Tool`、`Agent` 和 `Skill` 定义保持与参考实现兼容
- memory 第一版沿用参考实现的默认实现思路
- hooks 既要覆盖参考实现兼容的 runtime phase，也要覆盖 host 集成需求
- 框架必须提供一个内置主线程 routing agent，以及一组精简但实用的内置 tools 和 skills

参考实现源码显示，稳定的核心并不是 Ink UI，而是 bootstrap、session control、query loop、tool orchestration、memory 与 hooks 的组合。Python runtime 应当把这些层显式化，而不是像参考实现一样把大量职责隐含在 REPL 实现里。

## Goals / Non-Goals

**Goals:**

- 设计一个 Python runtime 架构，保持参考实现兼容的 tools、agents、skills、memory 与 hooks 语义。
...
- 不按行把参考实现从 TypeScript/Bun 迁移到 Python。
- 不保留 React/Ink 的渲染契约或终端组件内部实现。
- 第一版不支持用户自定义 memory backend；先只保留参考实现风格的文件型 memory。
- 不复刻参考实现内部所有 Anthropic 专用或产品专用 feature gate。
...
Python runtime 将保留参考实现面向用户的定义字段、加载规则和行为语义，但不会去模拟 TypeScript 类型系统或 Bun 特有基础设施。

Why:

- 这样能让已经基于参考实现心智模型工作的用户以更低成本迁移
- 可以避免 Python runtime 被 React、Bun 或 Anthropic 产品细节绑定
- 兼容目标聚焦在真正由用户编写的“定义层”

Alternatives considered:

- 重新设计一套 Python 原生定义模型。拒绝，因为这会直接丢掉参考实现兼容性和用户已有认知。
...
- 参考实现里这个角色已经隐式存在于主循环中，但不是一等 agent
...
### 5. 将参考实现的 memory 语义保留为默认 provider

第一版将实现一个默认文件型 memory provider，从参考实现的流程：
...
runtime 将支持参考实现兼容的 runtime hook phases，例如 `SessionStart`、`UserPromptSubmit`、`PreToolUse`、`PostToolUse`、`Stop`、`SubagentStop`、`SessionEnd`。同时也会增加 host lifecycle hooks，用于把 CLI 或 UI 的启动、关闭等逻辑纳入 runtime 主流程，而不是把这些逻辑硬编码到 turn engine 里。

Why:

- 参考实现的 hooks 是明显的 cross-cutting mechanism，参与验证、prompt augment、permission flow 与 cleanup
- host startup 逻辑属于 runtime integration，而不属于 conversation loop 本身
- 用统一事件总线建模 runtime 和 host 扩展，整体会更一致

Alternatives considered:

- 只保留参考实现兼容的 runtime hooks。拒绝，因为用户明确要求为 host 逻辑预埋 hook。
...
- 它们反映的是参考实现中通用且 runtime 级的能力，而不是产品专用辅助功能

Alternatives considered:

- 不提供任何 built-ins。拒绝，因为这样很难验证 runtime 的端到端可用性。
- 把参考实现的所有 built-ins 都搬过来。拒绝，因为其中很多能力是产品特定的，而不是 runtime 必需的。

## Risks / Trade-offs

- **[兼容性漂移]** Python 行为可能在大体语义上兼容参考实现，但在边界情况下仍有偏差。 → Mitigation: 尽早增加定义级 fixtures 和兼容性测试。
...
- **[host 耦合]** startup 与 shutdown hooks 如果处理不当，会把 host 关注点反渗入 runtime core。 → Mitigation: 区分 reference-compatible runtime events 与 host lifecycle hooks，但仍通过同一 hook bus 暴露。
...
- 如果 runtime 提取最终证明方案不可行，可以回滚实现层工作，而不会影响现有参考实现解读材料，因为这次变更新增的是一个独立的 Python runtime 面，而不是修改 `cc-src` 本身。
...
- turn engine 下层的 model client abstraction 应该如何设计，才能让参考实现兼容行为跨 provider 保持稳定？
