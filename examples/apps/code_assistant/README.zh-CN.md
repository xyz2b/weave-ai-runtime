...
这个 app 在早期 demo 的 durable live-runtime path 基础上，继续叠加交互式 `bash v2` 表面、响应式 runtime observability，以及 app-owned workflow ledger：
...
App-local `bash` replacement 保留了公开 tool 名称 `bash`，但 V2 契约支持：

- 现有短命令的 one-shot `exec`
- 面向长生命周期 shell session 的 `start`、`send`、`read`、`interrupt` 与 `stop`
- 共享 job visibility 与结构化 shell-session metadata
...
- `/inspect` 输出中的 `current transcript: local-shell`、`current task list: session:local-shell` 与至少一行 `workflow:` snapshot
...
Host 对 `edit`、`write` 和 `bash` 使用普通 permission path。
...
只有 `bash` 被这个 app 本地替换。V2 replacement 保留了公开 tool 名称 `bash`，同时增加：

- 面向 coding shell 工作的命令分类
- 更清晰的 workspace-aware guardrails 与高风险摘要
- 结构化 stdout/stderr 预览
- 面向长命令的后台或 session-oriented job projection
- 通过 `start`、`send`、`read`、`interrupt`、`stop` 实现的逐行 shell-session 生命周期
