...
- 这样可以保留“共享 turn engine”的统一性，同时补足参考实现风格的“上下文塑形”
...
- 参考实现的可取之处不是 spawn branch 数量，而是 branch 之后仍会归一到共享执行器
...
而 `model` 字段仍保留参考实现兼容语义，只用于覆盖 route 的默认模型名，不承担 transport 配置。
...
- 这能保留参考实现的 fork 设计里真正有价值的部分：prompt cache 复用
...
- 参考实现的可取之处在于 capability trimming 是 runtime contract，不是 prompt 约定
