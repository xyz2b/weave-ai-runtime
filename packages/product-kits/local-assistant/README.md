...
- host-centric local-assistant product-profile defaults layered on shared read-only web and bridge packages
...
- Choose `weavert-kit-local-assistant` when your app owns host approvals and needs read-only web lookup plus browser, local-machine, or PIM bridges as one higher-layer profile.
