...
for src_root in sorted(PROJECT_ROOT.glob("packages/**/src")):
...
def _capture_stream_requests() -> tuple[list[dict[str, Any]], Any]:
    ...
    def wrapped(url: str, payload: dict[str, Any], *, api_key: str):
        record = {
            ...
            "sample_tools": [tool.get("name") for tool in (payload.get("tools", []) or [])[:16]],
        ...
        for event in original(url, payload, api_key=api_key):
            if event.get("type") == "response.output_item.added" and event.get("item", {}).get("type") == "function_call":
                item = event["item"]
                record["stream_calls"].append({"call_id": item.get("call_id"), "name": item.get("name")})
...
async def _run_smoke(runtime) -> dict[str, Any]:
    ...
    try:
        async for event in runtime.stream_prompt(PROMPT, session_id=f"live-openai-smoke-{uuid4().hex[:8]}"):
                ...
                attempts.append(
                    {
                        ...
                        "fallback": event.attempt.metadata.get("stream_completed_output_fallback"),
                    }
                )
                continue
            ...
            if event.event_type == TurnStreamEventType.MESSAGE and event.message is not None:
                ...
                if message.role == MessageRole.ASSISTANT:
                    tool_uses = [
                        {"tool_use_id": block.tool_use_id, "name": block.name}
    ...
    checks = {
        "default_route_is_openai_default": runtime.kernel.config.default_model_route == openai_client.OPENAI_ROUTE_NAME,
        "parallel_tool_calls_requested": any(record.get("parallel_tool_calls") is True for record in captured),
        "multiple_sibling_tool_uses_observed": any(turn["tool_use_count"] > 1 for turn in assistant_tool_turns),
        "tool_use_attempt_observed": any(
            attempt["stop_reason"] == "tool_use" and attempt["produced_tool_calls"] for attempt in attempts
        ),
    ...
    if any(record["completed_output_len"] == 0 and record["stream_calls"] for record in captured):
        checks["empty_completed_output_fallback_observed"] = any(
            attempt.get("fallback") == "finalized_stream_tool_blocks" for attempt in attempts
        )
    ...
    return {
        ...
        "runtime": {
            ...
            "route_metadata": runtime.kernel.config.model_routes[runtime.kernel.config.default_model_route].metadata,
        },
        "checks": checks,
        "attempts": attempts,
