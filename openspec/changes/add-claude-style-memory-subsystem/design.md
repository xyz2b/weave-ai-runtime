...
当前 runtime 对 memory 只有占位数据模型，没有 Claude Code 风格的默认行为。Claude Code 的 memory 不是简单 KV store，而是一套明确绑在 session lifecycle 上的控制面语义：
...
如果第一版只做抽象 provider 或通用 KV store，用户接口虽然“有 memory”，但 runtime 行为不会像 Claude Code。

## Goals / Non-Goals

**Goals:**

- 把 Claude 风格 memory lifecycle 直接实现为 runtime 的一等子系统。
...
- 在第一阶段优先保证 Claude 语义对齐，而不是过早扩展 backend 自定义能力。
...
- Claude Code 的默认 memory 行为本来就是文件型、session-aware 的。
- 先稳定默认 provider，比一开始开放任意 backend 更符合当前目标。

Alternatives considered:

- 先做通用 KV store。拒绝，因为这无法表达 Claude 风格 `MEMORY.md` 与 scope 语义。
...
- 这与 Claude Code 的 memory lifecycle 一致，也最适合作为可测试 contract。
...
### 4. 第一版优先保持 Claude 默认语义，而不是过早开放自定义

第一版先把默认文件型 memory 与 scopes 做稳，再考虑后续用户自定义 backend。

Why:

- 当前目标是 Claude 对齐，不是 memory 平台化。
...
- `MEMORY.md` 之外是否需要第一阶段就支持额外 memory fragment 文件约定，还是先聚焦 Claude 默认入口？
