...
memory v1 已经把 Claude 风格的基础语义落地为 runtime control plane：
...
但它仍然只有一个中心化 `MemoryManager`。Claude Code 的更成熟之处不在于“使用了文件型 memory”，而在于它把记忆拆成不同时间尺度和不同作用范围的运行时层次。对于 framework 而言，v2 不应机械复刻 Claude 的每一步内部算法，而应保留 Claude 风格的外部心智模型，同时把内部实现升级为 layered architecture 与 mixed policy。
...
- 保留 Claude 风格的外部语义：文件型 memory、scope boundary、turn 前召回、turn 后抽取、后台整理。
...
- 不要求 v2 在内部逐步 1:1 复制 Claude Code 的 side-query 与 extractor 调度细节。
...
这意味着 v2 更像“Claude-compatible layered runtime”，而不是“Claude internals literal port”。
...
这与 Claude 的区别在于：Claude 倾向于把模型判相关放在 retrieval 主路径；v2 则把它放在最后一跳。
...
### 2. 完全复制 Claude 的 side-query 与 extractor orchestration
