...
- low-level `web_search`, `web_fetch`, and `web_find` primitives backed by `weavert-web-research`
...
Use `web_research` for AI-first research and pass `profile="coding"` or another supported profile when the scenario needs profile-specific source ranking or facets. Profile-specific details are returned under `facets.<profile>`.
