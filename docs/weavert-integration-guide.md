...
这里的 “canonical `weavert` public imports” 现在有一个更严格的边界：

- `weavert` 只保留 runtime core surface
- first-party add-on family 统一使用各自的 package root，例如 `weavert_openai`、`weavert_memory`、`weavert_hosts_reference`
- 启用 optional first-party package 时，推荐通过 distribution / package selection seam 装配，而不是继续把 `weavert.*` 当作这些 add-on 的别名
...
分发会决定哪些 first-party package 被装配，但不会改变 public import 边界：

- core-owned contract 继续从 `weavert` 导入
- non-core package implementation 则继续从各自的 `weavert_*` root 导入
...
其中 capability metadata 里列出的 `expected_tools` / `expected_agents` /
`expected_skills`，也依赖你把推荐 first-party packages 一并启用；单独 request
scenario pack 不会自动把这些 surface 注册进 runtime。
不过 scenario pack 本身仍然可以通过 package-owned context contributor 发布 profile guidance，
并在缺少推荐 first-party package 或默认省略 coding surface 时发布 contextual diagnostics。
更细的 package-surface contract 和 app-owned shell convention 见
`docs/weavert-user-extension-guide.md` 的 package-surface contract 小节。
