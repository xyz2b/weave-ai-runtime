...
Pro synthesis is evidence-bound. The synthesizer receives bounded ledger evidence, conflicts, gaps, freshness metadata, provider metadata, and the objective, then returns answer text plus structured claims. Runtime accepts only claims whose evidence ids exist in inspected ledger evidence. Accepted claims may preserve bounded metadata such as `claim_key`, `stance`, `conflicts_with`, `incompatible_with`, `resolved`, and `resolution_rationale`; unbound metadata cannot create public claims, evidence, conflicts, or high confidence. Optional excerpt spans or exact excerpts are validated when supplied; invalid span metadata is marked unsupported without making the model a source of truth.
