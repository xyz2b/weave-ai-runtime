...
从参考实现在 [query.ts](../../../../cc-src/query.ts#L203) 中显式维护的 `State`，我们的 turn loop 也必须有一个 loop-carried state，只是保持当前 Python runtime 的模块化拆分，而不是复制单文件巨型对象。规范上，`TurnLoopState` 至少包含：
...
这套 phase 划分直接吸收参考实现在 [query.ts](../../../../cc-src/query.ts#L369)、[query.ts](../../../../cc-src/query.ts#L560)、[query.ts](../../../../cc-src/query.ts#L1267) 上的 join-point 设计，但避免把所有逻辑都重新塞回一个 `query.ts`。
...
### 3D. continuation reason 与 recovery action 必须是显式枚举参考实现在 [query.ts](../../../../cc-src/query.ts#L1110)、[query.ts](../../../../cc-src/query.ts#L1162)、[query.ts](../../../../cc-src/query.ts#L1217)、[query.ts](../../../../cc-src/query.ts#L1246)、[query.ts](../../../../cc-src/query.ts#L1302)、[query.ts](../../../../cc-src/query.ts#L1338)、[query.ts](../../../../cc-src/query.ts#L1725) 中把 continuation 原因显式写进 `state.transition.reason`。我们的设计也要做到这一点。
...
第一阶段的实现落点必须直接对应到 [engine.py](../../../../src/runtime/turn_engine/engine.py)：

- [engine.py](../../../../src/runtime/turn_engine/engine.py) 中当前 host-facing `TurnStreamEventType.TERMINAL` 的语义必须改成“仅 turn-final”
- [engine.py](../../../../src/runtime/turn_engine/engine.py) 中当前用 `TERMINAL(stop_reason=\"tool_use\")` 表示 attempt 结束的做法必须移除
- [engine.py](../../../../src/runtime/turn_engine/engine.py) 必须保证 `max_turns`、`error`、`interrupted`、`blocked` 都走显式 final terminal 出口
- [engine.py](../../../../src/runtime/turn_engine/engine.py) 必须保证 final terminal 之后不再产出任何 turn event
...
### 5A. turn terminal reason 与 session status projection 必须显式定义参考实现在 [query.ts](../../../../cc-src/query.ts#L646)、[query.ts](../../../../cc-src/query.ts#L996)、[query.ts](../../../../cc-src/query.ts#L1051)、[query.ts](../../../../cc-src/query.ts#L1175)、[query.ts](../../../../cc-src/query.ts#L1264)、[query.ts](../../../../cc-src/query.ts#L1279)、[query.ts](../../../../cc-src/query.ts#L1515)、[query.ts](../../../../cc-src/query.ts#L1520)、[query.ts](../../../../cc-src/query.ts#L1711) 中把 turn terminal reason 写得很明确。我们的 runtime 也需要同样明确，但把 turn terminal 与 session status 投影拆开。
...
第一阶段的实现落点必须直接对应到 [agent_execution_service.py](../../../../src/runtime/agent_execution_service.py#L202)：

- [agent_execution_service.py](../../../../src/runtime/agent_execution_service.py#L202) 不能再使用 `turn_result.completed` 作为 child-run status 的主判断条件
- [agent_execution_service.py](../../../../src/runtime/agent_execution_service.py#L202) 必须改成读取显式 `TurnTerminalReason`
...
| --- | --- | --- | --- |
| [engine.py](../../../../src/runtime/turn_engine/engine.py) event surface | `TERMINAL` 同时承担 attempt-final 与 turn-final 语义 | `TERMINAL` 只表示 turn-final，attempt 结束改走 `ATTEMPT_FINISHED` 或等价非终态载体 | host、tests、controller 都会误判 turn 已结束 |
| [engine.py](../../../../src/runtime/turn_engine/engine.py) tool continuation | assistant 输出 `tool_use` 时仍先发 `TERMINAL(stop_reason=\"tool_use\")` | `tool_use` 只能是 attempt outcome，不得冒充 final terminal | continuation path 与终态 path 混淆 |
| [engine.py](../../../../src/runtime/turn_engine/engine.py) max-turn exit | `while iteration < max_iterations` 落出后没有显式 final terminal | `max_turns` 必须产出唯一 final `TurnTerminalReason` | child-run / host 无法稳定观察到真实终态 |
| [engine.py](../../../../src/runtime/turn_engine/engine.py) failure precedence | provider/model error 仍可能在 stop phase 被改写成 `blocked` | failure-class terminal 需要保留原语义，不能降级成 waiting/blocking | session 可能错误进入 `WAITING` |
| [controller.py](../../../../src/runtime/session_runtime/controller.py) terminal consumption | 任意 `TERMINAL` 都会被当成当前 turn 的最终终态 | 只能消费 turn-final terminal；attempt outcome 不应驱动 session 投影 | session memory/persistence 会基于错误终态运行 |
| [agent_execution_service.py](../../../../src/runtime/agent_execution_service.py#L202) child-run projection | `completed=False` 一律推断成 `MAX_TURNS` | child-run status 必须基于显式 `TurnTerminalReason` 映射 | `error` / `blocked` / `interrupted` 被错误记成 `max_turns` |
...
1. 先改 [engine.py](../../../../src/runtime/turn_engine/engine.py) 的事件面：
   收回 host-facing `TERMINAL`，引入 `ATTEMPT_FINISHED` 或等价 attempt-level carrier。
2. 再改 [engine.py](../../../../src/runtime/turn_engine/engine.py) 的退出路径：
   保证 `max_turns`、`error`、`interrupted`、`blocked` 全部产出唯一 final terminal。
3. 再改 `TurnResult` 聚合：
   把 `attempts[]` 与 `stop_reason` 的层级语义拆开，清理 `completed` 的派生规则。
4. 再改 [controller.py](../../../../src/runtime/session_runtime/controller.py) 和 [agent_execution_service.py](../../../../src/runtime/agent_execution_service.py#L202) 的状态投影：
...
- [controller.py](../../../../src/runtime/session_runtime/controller.py) 的 turn terminal 消费逻辑
