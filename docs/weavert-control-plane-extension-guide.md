...
如果你想先跑一个最小控制面例子，先看 `examples/README.md`。  
其中的 `session.hooks.on_pre_tool_use(...)` demo 和 general package demo 分别对应 layered session Hook registrar 与 `PackageContribution.context_contributors` 这两条稳定入口；本文继续负责解释 ownership、阶段语义和长期 contract。
...
当你的系统不是单纯“调一下 weavert.run_prompt()”，而是一个真正的宿主时，应实现或复用 `HostRuntime`。
...
- `HostRuntime.emit_team_event()` -> `HostRuntime.emit_extension_event(HostExtensionEvent(namespace="weavert.team", ...))`
...
- `RuntimeServices.resolve_team_*()` 在未装配 `weavert-team` 时返回 `None`
- `RuntimeAssembly.resolve_host_facet(RuntimeHostFacetKey.TEAM_WORKFLOWS.value)` 在未装配 `weavert-team` 时返回 `not_available`
- `weavert.team` namespace 的 extension event 只会在装配了 `weavert-team` 时出现
...
- metadata publication
  ...
  - `weavert.services.metadata["package_lookup"]["canonical_service_family_resolvers"]`
...
```python
weavert.services.hook_bus.register(
...
```python
weavert.services.hook_bus.register_handlers(
    session_id="session-1",
    owner="skill:rewrite",
    hooks={
        "PreToolUse": {
...
- 如果需要审计剩余 compat seam，直接看 `weavert.services.metadata["compatibility_boundaries"]`
- 如果需要看 closure / retirement / persistence / isolation 的总状态，直接看 `weavert.services.metadata["closure_report"]`
- 如果需要聚合 gate 结果，直接看 `weavert.services.metadata["protocol_only_conformance"]`
