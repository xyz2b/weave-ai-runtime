...
如果你只想先跑通一个最小示例，先看 `examples/README.md` 里的 `session.hooks.on_pre_tool_use(...)` demo。  
本文保留为 Hook registration model、phase contract 和诊断语义的说明文档；当前推荐 surface 是 layered hook registrars：`runtime.hooks`、`bound.hooks`、`session.hooks`，以及显式 advanced surface `session.hooks.advanced`。其中 host-owned hook authoring 的实际公开入口是 `bound = weavert.bind_host(host)` 返回的 `BoundHostRuntime`。agent-owned hooks 不是这里要鼓励的默认 authoring path。
...
对新的公开 authoring path，先把 surface 分三层理解：

- `session.hooks` / `runtime.hooks` / `bound.hooks`
  - simple callback-first stable surface
- `.typed`
  - phase-aware callback + effect-declaration surface，仍然编译到 canonical request
- `.raw`
  - 直接提交 canonical `HookRegistrationRequest`

helper builder 仍然保留，但默认推荐先看 layered simple surface。它本质上还是会生成普通 `HookRegistrationRequest`，只是把常见 callback、matcher 和 scope 组装步骤收起来：
...
如果你已经需要 turn-owned lifetime，再显式切到：
...
什么时候优先用 default layered simple / helper path：
...
`host-owned bound API`

- 适合宿主层统一挂载的 Hook，例如模型路由、审计、企业策略。

```python
handle = bound.hooks.typed.on_pre_model_request(
    lambda _payload: {"request_override": {"requested_model": "enterprise-model"}},
    effects=({"effect_fields": ["request_override", "metadata"]},),
)
```

这里有一个 adopter 最容易忽略的点：

- `bound.hooks...` 默认不是“立刻给所有现有 session 挂一个 active hook”。
...
- 如果你只是想快速写一个最小可运行 Hook，优先从 `session.hooks` 开始；只有在需要 advanced phase 或 turn-owned lifetime 时，再切到 `session.hooks.advanced`。
...
| `host API` | `bound.hooks.on_pre_tool_use(...)` / `bound.hooks.typed...` / `bound.hooks.raw.register(...)` | `session-template` | 宿主统一挂载的企业策略、路由、审计 | stable public |
| `session API` | `session.hooks.on_pre_tool_use(...)` / `session.hooks.typed...` / `session.hooks.raw.register(...)` | `session` | 某个 session 内持续生效的动态逻辑 | stable public |
| `turn API` | `session.hooks.advanced.turn...` 或 `session.register_turn_hook(...)` | `turn` | 只影响当前 turn 的一次性或短期逻辑 | advanced |
...
- 想让宿主统一注入策略，用 `bound.hooks`。
- 想在一次会话里动态开关能力，用 `session.hooks`。
- 想只影响当前 turn，优先用 `session.hooks.advanced.turn`。
...
session.hooks.advanced.on_recovery_decision(
    lambda _payload: {
        "continue_execution": True,
        "injected_messages": ["Approval received"],
    },
    effects=({"effect_fields": ["continue_execution", "injected_messages", "metadata"]},),
)
```
...
2. 默认选择 `session.hooks` 这条 simple callback-first stable surface。
3. 只有在需要 phase-aware effect authoring 时再切到 `.typed`；只有在需要 raw canonical control 或 advanced / turn-owned path 时再切到 `.raw` 或 `.advanced`。
...
```python
from weavert.runtime_kernel import RuntimeConfig, assemble_runtime
from weavert.hooks import HookActivationState, HookInventoryQuery, block_execution, match_tool
...
handle = session.hooks.raw.register(
    HookRegistrationRequest(
        phase="PreToolUse",
        match={"target": "deploy"},
...
如果你明确要做 `turn` 级 Hook，优先直接切到 `session.hooks.advanced.turn...`；只有在你已经完整展开 raw request 时，才需要自己再去写 `HookScopeLifetime.TURN` 和 `turn_id`。
...
- 从 `session.hooks...` 升级到 `runtime.hooks...` 或 `RuntimeConfig(hooks=...)`，把它变成默认模板 Hook。
