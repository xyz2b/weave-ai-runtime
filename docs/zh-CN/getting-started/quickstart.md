...
- 先完成 `installation.md`
- 如果你是在这个仓库的本地 checkout 里工作，请改用 `install-from-source.md`
...
如果你是在本地 source checkout 里工作，请改用 `install-from-source.md`，让同一套 baseline 从 editable local roots 提供。

可选的 scenario kits 可以以后再按需安装。
例如面向编码场景的 product defaults 已发布为 `weavert-kit-coding`。
