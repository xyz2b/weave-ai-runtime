...
- [ ] 1.1 Add session metadata / private-context support for dynamic skill roots and resume-safe activation evidence.
  Owner modules: `src/claude_agent_runtime/session_runtime/controller.py`, `src/claude_agent_runtime/contracts.py`, `src/claude_agent_runtime/runtime_kernel/kernel.py`.
...
- [ ] 1.2 Implement de-duplicated upward discovery of nested `.claude/skills` roots from observed workspace paths.
  Owner modules: `src/claude_agent_runtime/runtime_kernel/kernel.py`, `src/claude_agent_runtime/tool_orchestration.py`, `src/claude_agent_runtime/registries/discovery.py`.
  ...
  Dependencies: 1.1.
  Acceptance: observing a file under a nested subtree discovers the closer `.claude/skills` root during the same session without duplicating broader roots.
- [ ] 1.3 Add a runtime-kernel cache for parsing skill definitions from discovered roots.
  Owner modules: `src/claude_agent_runtime/runtime_kernel/kernel.py`, `src/claude_agent_runtime/registries/discovery.py`.
...
- [ ] 1.4 Build merged effective skill views that preserve existing source precedence and prefer deeper roots within the same source class.
  Owner modules: `src/claude_agent_runtime/runtime_kernel/kernel.py`, `src/claude_agent_runtime/registries/skill_registry.py`, `src/claude_agent_runtime/definitions.py`.
...
- [ ] 2.1 Extend invocation resolution to track discovered roots, activation eligibility, and richer hidden reasons.
  Owner modules: `src/claude_agent_runtime/definitions.py`, `src/claude_agent_runtime/invocation_catalog.py`.
...
- [ ] 2.2 Unify host-visible, model-visible, and executable skill gating on top of one resolved invocation decision path.
  Owner modules: `src/claude_agent_runtime/invocation_catalog.py`, `src/claude_agent_runtime/turn_engine/engine.py`, `src/claude_agent_runtime/runtime_kernel/kernel.py`.
...
- [ ] 2.3 Add regression tests for nested skill discovery, same-name shadowing, path mismatch, and hidden diagnostic reporting.
  Owner modules: `tests/test_discovery.py`, `tests/test_invocation_catalog.py`.
  State contract: test fixtures must exercise nested `.claude/skills` roots and root-aware diagnostics rather than only flat skill dirs.
...
- [ ] 2.4 Add resume-path tests ensuring observed-path activation and discovered roots are restored after transcript resume.
...
- [ ] 3.1 Introduce a dedicated skill request override state for `model` and `effort`.
  Owner modules: `src/claude_agent_runtime/contracts.py`, `src/claude_agent_runtime/skill_runtime.py`, `src/claude_agent_runtime/turn_engine/engine.py`.
...
- [ ] 3.2 Apply inline skill overrides to the next request build with consume-once semantics.
  Owner modules: `src/claude_agent_runtime/skill_runtime.py`, `src/claude_agent_runtime/turn_engine/engine.py`.
...
- [ ] 3.3 Implement field-level last-write-wins behavior when multiple inline skills shape one pending request.
  Owner modules: `src/claude_agent_runtime/skill_runtime.py`, `src/claude_agent_runtime/turn_engine/engine.py`.
...
- [ ] 3.4 Forward forked skill overrides into child `AgentInvocation` without mutating parent pending request state.
  Owner modules: `src/claude_agent_runtime/skill_runtime.py`, `src/claude_agent_runtime/agent_runtime.py`.
...
- [ ] 4.1 Extract a shared skill prompt expander for argument, session, and skill-directory variable substitution.
  Owner modules: `src/claude_agent_runtime/skill_runtime.py` or a sibling expander module under `src/claude_agent_runtime/`.
...
- [ ] 4.2 Add Claude-compatible shell block parsing for local file-backed skills.
  Owner modules: `src/claude_agent_runtime/skill_runtime.py` or the extracted expander module.
  ...
  Dependencies: 4.1.
  Acceptance: local skills with Claude-compatible inline or fenced shell blocks are parsed for execution, while non-local sources surface an expansion error.
- [ ] 4.3 Execute skill shell blocks through the existing shell tool path, honoring `shell` frontmatter and current permission constraints.
  Owner modules: `src/claude_agent_runtime/skill_runtime.py`, existing shell tool execution path, `src/claude_agent_runtime/turn_engine/engine.py`.
...
- [ ] 4.4 Surface shell expansion failures, permissions, telemetry, and observed paths through the existing runtime channels.
  Owner modules: `src/claude_agent_runtime/skill_runtime.py`, `src/claude_agent_runtime/tool_orchestration.py`.
...
- [ ] 5.1 Add end-to-end tests for inline skill model / effort overrides and forked child override propagation.
...
- [ ] 5.2 Add end-to-end tests for non-user-invocable skills, path-ineligible user invocations, and policy-narrowed skill execution.
  Owner modules: `tests/test_invocation_catalog.py`, `tests/test_agent_skill_runtime.py`.
...
- [ ] 5.3 Add end-to-end tests for successful shell expansion and fail-closed shell error paths.
  Owner modules: `tests/test_agent_skill_runtime.py`.
...
- [ ] 5.4 Verify the full change with targeted `pytest` coverage for discovery, invocation catalog, and skill runtime behavior.
