...
| `weavert-kit-common-web-research` | `weavert_kit_common_web_research` | `weavert-shared-web-research` | Unified read-only web research surfaces with `web_research`, profile facets including `medical`, provider metadata, source/evidence tiers, Bing grounding/Google/SerpAPI/Brave/DuckDuckGo search selection, freshness outcomes, single-page `web_fetch`, and low-level `web_*` primitives | Add web information retrieval without taking a full scenario profile |
...
- `weavert-kit-common-web-research` owns the recommended `web_research` entrypoint for chat, coding, local-assistant, and other profile-driven public-web research. Use `profile` to select strategy; profile-specific fields appear under `facets.<profile>`. `legal_compliance` and `medical` default to hard freshness, while `business` and `product_shopping` treat freshness as soft unless explicitly required.
- `weavert-kit-common-web-research` performs read-only public-web search, single-page page fetch, page-local evidence finding, and multi-page inspection behind `web_research`. Provider selection is handled by the shared core and can use Azure AI Foundry Bing grounding (`bing-grounding`), Google Programmable Search (`google-search`), SerpAPI Google Search (`serpapi-google-search`), Brave, or DuckDuckGo without changing public tool names. Source records may include `source_tier` and evidence records may include `evidence_tier` values from `official`, `authoritative`, `media_report`, `single_source_report`, and `general`. It does not expose browser navigation, clicks, public batch-fetch fields, or host-side browser control.
