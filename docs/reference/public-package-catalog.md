...
| `weavert-kit-common-web-research` | `weavert_kit_common_web_research` | `weavert-shared-web-research` | Unified read-only web research surfaces with `web_research`, profile facets, provider metadata, Bing grounding/Google/Brave/DuckDuckGo search selection, freshness outcomes, single-page `web_fetch`, and low-level `web_*` primitives | Add web information retrieval without taking a full scenario profile |
...
- The retired `weavert-kit-common-web` distribution is no longer part of the public package set. Use `weavert-kit-common-web-research` for read-only public-web search, fetch, find, and profile-driven research surfaces.
- `weavert-kit-common-web-research` performs read-only public-web search, single-page page fetch, page-local evidence finding, and multi-page inspection behind `web_research`. Provider selection is handled by the shared core and can use Azure AI Foundry Bing grounding (`bing-grounding`), Google, Brave, or DuckDuckGo without changing public tool names. It does not expose browser navigation, clicks, public batch-fetch fields, or host-side browser control.
