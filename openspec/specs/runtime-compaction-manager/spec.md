...
The runtime SHALL provide a unified compaction manager that is invoked before provider requests when runtime context pressure or compaction policy requires it.
...
The runtime SHALL preserve resume-safe semantics when compaction modifies session context.
