...
> Documentation note: This file remains a maintainer-facing ledger. Start with `docs/maintainers/validation-findings.md` for the index of validation notes.

This file now only keeps the repo-owned demo findings ledger: which user-centric validation layer examples exposed which follow-up seams while the examples were being authored.
If you are new to the repo, read `examples/README.md` first and treat this page as evidence, not as a starting guide or an implied roadmap commitment.
