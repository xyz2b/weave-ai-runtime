...
- 定义比当前更接近 Claude Code 的模型流协议和 turn stream 协议。
...
- 本 change 不追求一次性复刻 Claude Code 的所有产品遥测和 feature gate。
...
- Claude Code 里 `claude.ts` 和 `query.ts` 也是分层的：一个处理 provider raw stream，一个处理 turn loop。
...
- Claude Code 的 host 实际上消费的是 `query()` yield 出来的统一事件流。
...
- Claude Code 明确避免把半截 thinking/tool input 留在下一轮上下文中。
