...
- `kernel public` 更偏运行时主流程和 tool / session / subagent 生命周期。
- `control-plane public` 更偏上下文装配、模型请求整形、响应处理和恢复决策。
...
下面这个 API 片段假定当前 session 已经处于活跃 turn 中，因此可以直接使用 `turn` 级 scope：
...
同一套 Hook 模型可以从不同来源进入系统，区别主要在“谁拥有它”“默认生命周期是什么”“适合什么场景”。

| 注册来源 | 典型入口 | 默认作用范围 | 适合场景 | 备注 |
| --- | --- | --- | --- | --- |
| `runtime config` | `RuntimeConfig(hooks=...)` | `session-template` | 所有 session 默认生效的基础能力 | 适合平台级默认策略 |
| `host API` | `host.register_hook(...)` | `session-template` | 宿主统一挂载的企业策略、路由、审计 | 更偏宿主接入面 |
| `session API` | `session.register_hook(...)` | `session` | 某个 session 内持续生效的动态逻辑 | 最常见的运行时接入方式 |
| `turn API` | `session.register_hook(...)` 或 `session.register_turn_hook(...)` | `turn` | 只影响当前 turn 的一次性或短期逻辑 | 适合局部控制，但要求存在活跃 turn |
| `legacy definition` | 旧版 `hooks.PreToolUse.matcher/effect` | 取决于加载路径 | 兼容历史 definition | 新设计不推荐继续扩展 |
...
公开 manifest 模型支持 `callback`、`http`、`command`、`agent` 和 `prompt`。其中外部 handler 默认是拒绝的，必须通过 `HookBus.set_handler_policy(...)` 显式放行。
...
3. 在当前 `session` 上动态注册一个 `turn` 或 `session` 级 Hook。
...
runtime = assemble_runtime(RuntimeConfig())
session = runtime.create_session(session_id="demo-hook-session")
...
```python
from runtime.hooks import HookInventoryQuery
...
```python
from runtime.hooks import HookDispatchTraceQuery
