...
- `TaskManager`
  - 继续作为 runtime 内部 background execution registry
  - 跟踪 background agent、memory job、teammate projection 等运行态记录
- `TaskListService`
  - 作为 runtime-owned planning control plane
  - 持久化 `task_create` / `task_get` / `task_update` / `task_list` 对应的共享 checklist
...
- `job_*`
  - 只表示 background-job query / stop 语义
...
flowchart TB
    ...
    subgraph Control["Control Plane Services"]
        ...
        Tasks["TaskManager / TranscriptStore"]
    end
