...
from .reference_coding_tool_impls import (
    git_diff_tool,
    git_history_tool,
    git_status_tool,
    ...
    return (
        SkillDefinition(
            ...
            content=(
                ...
                "6. Run verification through `bash`, using session actions or jobs only when the command is longer-lived.\n"
                "7. Run the explicit verification phase and then the explicit review phase before the final summary.\n"
                "8. Make sure verifier output starts with `verification: pass` or `verification: fail`.\n"
                "9. Make sure reviewer output starts with `review: pass` or `review: fail`.\n"
                "10. Finish with a concise summary naming changed files, verification, review, and any remaining workflow gaps."
            ),
