...
_TRANSIENT_MARKERS = ("temporary", "temporarily", "one-off", "for this turn", "for this pass")
...
def _looks_transient_task(text: str) -> bool:
    ...
    if any(lowered.startswith(prefix) for prefix in _TRANSIENT_PREFIXES):
        return any(word in lowered for word in _TRANSIENT_ACTION_WORDS)
...
def _conflict_key(fact_type: str, text: str, *, agent_name: str | None = None) -> str:
    ...
    if fact_type == "agent_workflow" and agent_name is not None:
