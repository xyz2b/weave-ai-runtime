...
- `LongTermMemory`
  - 共享 durable memory，落在 `.claude/memory/documents/`
  - 主要承载 `preferences/`、`conventions/`、`topics/` 和 `shared/`
- `AgentNamespaceMemory`
  - agent 私有 durable memory，落在 `.claude/memory/agents/<agent>/documents/`
  - 不越出当前 `user/project/local` boundary
- `SessionMemory`
  - 当前 session 的 continuity artifacts，落在 `.claude/memory/sessions/<session>/`
  - 主要包括 `session-summary.md`、`open-threads.md`、`metadata.json`
- `ConsolidationMemory`
  - 跨 session 的慢速后台整合层
  - 工作状态落在 `.claude/memory/consolidations/`
