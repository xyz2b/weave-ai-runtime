...
- 内置 `agent` tool 只有 `agent`、`prompt`、`background` 三个输入，child execution shaping 能力明显弱于 Claude Code。
...
这样做的原因是当前 execution contract 已经足够接近 Claude 风格 sidechain，只差落地和查询面。继续把 child 历史塞进主 transcript 会污染主线程上下文，也不利于 background/fork 观测。
