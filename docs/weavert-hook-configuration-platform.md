...
本文保留为 Hook registration model、phase contract 和诊断语义的说明文档；当前推荐 surface 是 layered hook registrars：`runtime.hooks`、`host.hooks`、`session.hooks`，以及显式 advanced surface `session.hooks.advanced`。agent-owned hooks 不是这里要鼓励的默认 authoring path。
...
- `session.hooks` / `runtime.hooks` / `host.hooks`
...
```python
handle = host.hooks.typed.on_pre_model_request(
...
- `host.hooks...` 默认不是“立刻给所有现有 session 挂一个 active hook”。
...
| `host API` | `host.hooks.on_pre_tool_use(...)` / `host.hooks.typed...` / `host.hooks.raw.register(...)` | `session-template` | 宿主统一挂载的企业策略、路由、审计 | stable public |
