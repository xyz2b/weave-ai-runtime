...
当前 Python runtime 的测试主要通过 fake model 批次断言“有没有第二次请求”和“最后文本是否匹配”。这种覆盖不足以证明它实现了 Claude Code 的 query runtime contract，因为真正关键的是：

- 第二轮 request 是否带了正确的 `tool_result.tool_use_id`
- interrupt 是否能终止 in-flight stream
- transcript 恢复后是否仍然维持合法 pairing
- model-generated `agent` / `skill` tool 是否走 assembled runtime 主路径

Claude Code 的 query runtime 本身就依赖大量消息结构与恢复逻辑，因此这里需要一个单独的 conformance change，用更接近协议层的 golden fixtures 锁住边界。
...
- 本 change 不覆盖所有 Claude Code feature gate，只覆盖当前四步拆分后的主骨架。
