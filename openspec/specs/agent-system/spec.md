...
### Requirement: Claude 兼容的 agent 定义
runtime SHALL 支持与 Claude Code 兼容的 agent 定义语义，包括 tools、disallowed tools、skills、model selection、effort、permission mode、max turns、background execution、memory scope 与 isolation。

#### Scenario: 注册自定义 agent 定义
- **WHEN** 用户使用 Claude Code 兼容字段定义一个 agent
