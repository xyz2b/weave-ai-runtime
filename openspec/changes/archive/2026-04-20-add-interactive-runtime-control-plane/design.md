...
当前 runtime 已经有可运行的 execution plane，但 interactive control plane 仍然散落在局部 callback 与 lifecycle stub 里。Claude Code 之所以能把 CLI、UI 与 SDK 收敛到一套运行时上，核心不在表层 while loop，而在一套真正参与 runtime flow 的 `HookBus`、permission system、elicitation pipeline 与 host bridge。
...
- 定义 Claude 兼容 runtime hook phases、structured effects、permission modes、waiting/resume control flow 与 host-mediated interaction contract。
- 为 skill hooks、subagent permissions、future UI adapters 与 notification streaming 提供稳定边界。

**Non-Goals:**

- 不复刻 Claude Code 的具体 terminal UI、Ink rendering 或 channel relay 产品逻辑。
...
- 这三块共同定义 Claude 风格 interactive runtime 的骨架。
...
- Claude Code 的 hooks 是会影响 runtime flow 的 cross-cutting mechanism，而不只是 prompt augmentation。
...
- Claude Code 的 permission 不是工具私有逻辑，而是 runtime control plane。
...
- Claude Code 的 elicitation 是 runtime 与 host 之间的通用交互能力，而不是单个 built-in tool。
...
- Claude Code 的可复用 session/turn stack 位于 host 之下。
...
- Claude Code 里 skill hooks、tool hooks、approval flow 与 `ask_user` 在运行时上是贯通的。
...
- **[接口过宽]** 一次性引入 hook、permission、elicitation、host bridge 容易让 control plane 变重。 → Mitigation: 第一版只覆盖 Claude 对齐所需核心交互点，不提前开放产品层扩展。
...
- `PermissionRequest`、`Notification`、`ElicitationResult` 等扩展 phases 是否应在第一阶段一并进入 `HookBus`，还是先覆盖 Claude 兼容核心 phases？
